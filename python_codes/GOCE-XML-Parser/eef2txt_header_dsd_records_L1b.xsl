...
<xsl:stylesheet id="header_dsd_records" version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="*/Earth_Explorer_Header/Variable_Header/SPH">
    ...
    <xsl:variable name="Output_Data_Set_Type" select="'M'"/>
    ...
    <xsl:variable name="DSDs" select="List_of_DSDs"/>

    <xsl:variable name="Start_GPS_Time"  select="normalize-space(Start_GPS_Time)"/>
    <xsl:variable name="Stop_GPS_Time"   select="normalize-space(Stop_GPS_Time)"/>
    <xsl:variable name="Abs_Orbit_Start" select="normalize-space(Abs_Orbit_Start)"/>
    <xsl:variable name="Abs_Orbit_Stop"  select="normalize-space(Abs_Orbit_Stop)"/>

    <xsl:for-each select="$DSDs/DSD">

      <xsl:variable name="output_product" select="Data_Set_Name"/>
      <xsl:variable name="set_type"       select="Data_Set_Type"/>

      <xsl:if test="$set_type = 'R' or 
                    $set_type = 'M' and 
                    $output_product = 'EGG_GGT_1i' or $output_product = 'EGG_CCD_1i' or $output_product = 'EGG_IAQ_1i'
                 or $output_product = 'SST_COV_1i' or $output_product = 'SST_PVT_1i'
                 or $output_product = 'STR_VC2_1i' or $output_product = 'STR_VC3_1i' ">
        ...
        <xsl:call-template name="Fixed_Length_Field">
          <xsl:with-param name="Field_Value" select="Filename"/>
        ...
        <xsl:choose>
          <xsl:when test="not(normalize-space(Num_DSR)='')">
            <xsl:call-template name="Fixed_Length_Field">
              <xsl:with-param name="Field_Value" select="Num_DSR"/>
