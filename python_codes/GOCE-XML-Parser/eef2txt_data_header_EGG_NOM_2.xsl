...
<!--
Stylesheet Name: EGG_NOM_2_data_header
Version: 1.1
Date: 14 Jul 2008
-->

<xsl:stylesheet id="EGG_NOM_2_data_header" version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="*/Earth_Explorer_Header/Variable_Header/SPH">
    ...
    <xsl:if test="not(starts-with($Mode, 'data_block'))">
      <xsl:apply-templates select="EGG_NOM_2" mode="header"/>
    </xsl:if>

  </xsl:template>

  <xsl:template match="*/Earth_Explorer_Header/Variable_Header/SPH/EGG_NOM_2" mode="header">
