function [X, Px, Xc, P_grav, h, ax] = do_SLAM_v4(time, state_true, planetParams, ...
    ...
    fprintf('\nProgress:   0%%'); count = 0; flag_count = 0; kmin = 1;
    for k = 2:Nt
        % initial conditions
        ...
        if(flag)
            Y_cal   = Y(:, 1:k) - X(7:end, 1:k);

            [Xg_smooth, Pg_smooth, NSM_err] = gravEstim_smoothing(time(1:k), Y_cal, ...
                planetParams, poleParams, X(:, 1:k), P0c_grav, ...
                Cnm, Snm, R0, Px(1:k, :));
            ...
            P0_smooth = reshape(Px(kmin, :), [Ns, Ns]);
            Pc_grav(2:end, 2:end)  = Pg_smooth;
    
            % run EKF
            [Xs_smooth, Ps_smooth] = EKF_smoothing(time(kmin:k), planetParams, ...
                poleParams, Y(:, kmin:k), X(:, kmin), P0_smooth, ...
            ...
            for j = 1:Nbatch
                ...
                Px(j+kmin-1, :) = reshape(b(1:Ns, 1:Ns), [Ns*Ns, 1]);
            end
            X(:, kmin:k)  = Xs_smooth;
            X0         = X(:, k);
            ...
            % update measurement arc initial point 
            kmin = k+1;
        end
