...
struct SimulationConfig {
    ...
    double area = 10.0;                   // Spacecraft area [m^2]
    double eta = 1.0;                     // SRP scale factor [-]
    int n_maxM = 10;                      // Moon SH max degree/order
    int n_maxE = 0;                       // Earth SH max degree/order
    string epoch_start = "2015 APR 29 00:00:00 UTC";
    string epoch_end   = "2015 APR 29 01:00:00 UTC";
    double fs = 1.0;                      // Sampling frequency [Hz]

    string output_file = "trajectory_output.txt";
};
...
State EOM_LRO_EPHEM(double t,
    ...
    // Precompute body-frame vectors before the parallel region.
    // CSPICE calls are intentionally kept outside OpenMP because CSPICE should
    // not be called concurrently from multiple threads.
    Vec3 r1_EarthFrame  = mat3_vec(EARTH_J2000, r1);
    Vec3 rME_EarthFrame = mat3_vec(EARTH_J2000, r_ME);
    Vec3 r2_MoonFrame   = mat3_vec(MOON_J2000, r2);

    // Keep the EOM-level operations serial. The only OpenMP work is inside
    // potentialGradient_nm_parallel_moon(), where the Moon SH summation is
    // distributed across threads. This avoids parallel overhead for cheap terms
    // and avoids concurrent CSPICE calls.
    double F = shadow_function(p.R_S, p.R_M, Spos, r2);
    SRPOutput srp = SRP(r3, p.eta, p.mass, p.area);

    GravityOutput Earth = potentialGradient_nm(
        C_E, S_E, p.n_maxE, r1_EarthFrame, p.R_E, p.GM_E, p.normalized
    );

    GravityOutput Earth_tide = potentialGradient_nm(
        C_E, S_E, p.n_maxE, rME_EarthFrame, p.R_E, p.GM_E, p.normalized
    );

    GravityOutput Moon = potentialGradient_nm_parallel_moon(
        C_M, S_M, p.n_maxM, r2_MoonFrame, p.R_M, p.GM_M, p.normalized
    );

    GravityOutput Sun = potentialGradient_nm(
        C_M, S_M, 0, r3, p.R_S, p.GM_S, p.normalized
    );
...
struct IntegratorOptions {
    ...
    double initialStep = 1;   // Initial internal step-size guess [s]
    double minStep = 1.0e-12;   // Minimum allowed internal step [s]
    double maxStep = 30.0;      // Maximum allowed internal step [s]
...
SimulationConfig read_simulation_config(const string& filename) {
    ...
    cfg.mass = get_required_double(kv, "mass");
    cfg.area = get_required_double(kv, "area");
    cfg.eta = get_required_double(kv, "eta");
    cfg.n_maxM = get_required_int(kv, "n_maxM");
    cfg.n_maxE = get_required_int(kv, "n_maxE");
    cfg.epoch_start = get_required_string(kv, "epoch_start");
    cfg.epoch_end = get_required_string(kv, "epoch_end");
    cfg.fs = get_required_double(kv, "fs");
    cfg.output_file = get_required_string(kv, "output_file");

    if (cfg.mass <= 0.0) throw runtime_error("Config mass must be positive.");
    if (cfg.area < 0.0) throw runtime_error("Config area must be nonnegative.");
    if (cfg.fs <= 0.0) throw runtime_error("Config fs must be positive.");
...
int main(int argc, char* argv[]) {
    try {
        ...
        cout << "OpenMP enabled with up to " << omp_get_max_threads()
             << " threads for the Moon SH summation." << endl;
#else
        ...
        // Earth is kept as point mass in this version. If n_maxE > 0, you must
        // add Earth C/S coefficient files and load them here.
        if (p.n_maxE > 0) {
            throw runtime_error("n_maxE > 0 requested, but this code currently does not load Earth SH coefficient files.");
        }
        ...
        str2et_c(cfg.epoch_start.c_str(), &tmin);
        str2et_c(cfg.epoch_end.c_str(), &tmax);

        if (tmax <= tmin) {
            throw runtime_error("epoch_end must be later than epoch_start.");
        }

        double fs = cfg.fs;
        int N = static_cast<int>(round((tmax - tmin) * fs)) + 1;
        vector<double> time = linspace(tmin, tmax, N);

        cout << "Loaded config file: " << configFile << endl;
        cout << "Mass [kg] = " << p.mass << endl;
        cout << "Area [m^2] = " << p.area << endl;
        cout << "Eta [-] = " << p.eta << endl;
        cout << "Moon SH degree = " << p.n_maxM << endl;
        cout << "Earth SH degree = " << p.n_maxE << endl;
        cout << "Sampling frequency [Hz] = " << fs << endl;
        cout << "Simulated time [s] = " << (tmax - tmin) << endl;
        cout << "Number of output samples = " << N << endl;
        ...
        odeOpts.absTol = 1.0e-8;
        odeOpts.initialStep = 1;
        odeOpts.minStep = 1.0e-12;
        ...
        save_trajectory_txt(cfg.output_file, time, state);
