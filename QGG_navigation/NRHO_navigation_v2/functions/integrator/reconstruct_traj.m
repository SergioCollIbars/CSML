function [X_new, STM] = reconstruct_traj(TIME_meas, TIME_obs, ddU_ACI, X0, ...
    enviroment, order)
...
%                                                                         %
%   Inputs: TIME_meas: gradiometer meas time vector                       %
%           TIME_obs: time vector to evaluate meas                        %
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % loop
    for j = 1:Nt-1
        % find meas time index equals observation time
        id  = TIME_meas == TIME_obs(j);
        % construct Jacobian
        T = reshape(ddU_ACI(:, id), [3,3]);
        if(enviroment == "CR3BP")
            J = [zeros(3, 3), eye(3,3); T, [0,2,0;-2,0,0;0,0,0]];
        elseif(enviroment == "2BP" || enviroment == "F2BP")
            J = [zeros(3, 3), eye(3,3); T, zeros(3, 3)];
        end

        % integrate PHI
        t_span = [TIME_obs(j), TIME_obs(j+1)];
        At = diff(t_span);
