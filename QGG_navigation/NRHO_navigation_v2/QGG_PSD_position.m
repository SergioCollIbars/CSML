...
function [F, T, PSD] = compute_spectrogram(fs, T_orbit, signal)
    ...
    frac = 1/10;                               % try 1/4–1/8 orbit windows
...
function [] = plot_spectrogram(F, T, PSD, T_orbit, sgtt)
    ...
    pcolor(T(1, :)./86400, F(1, :), 10*log10(x)); 
    axis xy; colormap turbo; cb = colorbar;
    shading flat
    set(gca, 'YScale', 'log');    % Make y-axis log
    xlabel('Time [days]'); ylabel('Frequency [Hz]');
    title('Gradiometer spectrogram in \Gamma_{xx} direction')
