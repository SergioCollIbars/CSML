...
traj0 = load_initCond(planetParams, TIME);
[b0_GG, ~] = load_bias(planetParams);
...
[posE, posM, posS] = compute_posPrimaries(TIME, planetParams);
[BN_matrix_true] = compute_attitude_EPHEM(TIME, frec, 1, "inertial");
[BN_matrix_meas] = compute_attitude_EPHEM(TIME, frec, 0, "inertial");
[T, B] = compute_measurement_EPHEM(TIME, state, planetParams, ...
    BN_matrix_true, Cmat_true, Smat_true, 1, posE, posM, posS);
state(:, 7:12) = B;

r_sc_moon = vecnorm(state(:, 1:3)' - posM);
if(plotResults), plot_measurements(TIME, [T;zeros(3, length(time))], ...
        planetParams, r_sc_moon); end
disp('  DONE!')

% initialize filter. Add error to a priori states
frec = 1 / (TIME(2) -TIME(1)) * planetParams(3);
