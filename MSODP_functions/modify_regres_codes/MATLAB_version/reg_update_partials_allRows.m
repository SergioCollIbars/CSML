function info = reg_update_partials_allRows(inReg, outReg, parCols, newPartials, opts)
%REG_UPDATE_PARTIALS Overwrite selected parameter partial columns in a binary .reg
% file, preserving everything else.
%
% info = reg_update_partials(inReg, outReg, parCols, newPartials)
%
% Inputs
%   inReg       : input .reg (binary)
%   outReg      : output .reg (binary)
%   parCols     : 1xK parameter indices (1..npar) to overwrite in each obs record
%   newPartials : Nobs x K matrix. Row i replaces partials for obs i at parCols.
%
% opts (optional)
%   opts.extraDoubles          : default 5 (matches your C code)
%   opts.partialsStartInRecord : default extraDoubles+1
%   opts.igrpsAtEnd            : default true  (record = [extra][partials][igrps])
%   opts.endianness            : default "ieee-le" (change to "ieee-be" if needed)
%   opts.formatCheckOnly       : if true, reads header and stops (no write)
%
% Output
%   info.npar, info.rowsize, info.nObs, etc.
   ...
   arguments
        ...
        opts.partialsStartInRecord (1,1) double {mustBeInteger, mustBePositive} = 1 % <-- placeholder
        opts.igrpsAtEnd (1,1) logical = true
        opts.endianness (1,:) char = 'ieee-le'
        opts.formatCheckOnly (1,1) logical = false
    end

    % Set dependent default here (LEGAL)
    ...
    % Helpers to read/write with exact types
    ...
    % Station block (copied exactly like your C)
    if nsta ~= 0
    ...
    if igrps > 0
        ...
        sescnt = sum(double(igrpcnt));
        sestot = sum(double(igrptot));
    end

    % Satellite ids + stasat
    ...
    % Parameter arrays (apriori values, names, sigmas, scales)
    ...
    % We will stream records until EOF, like your C.
    nObs = 0;

    % Determine where partials start in the record vector
    p0 = double(opts.partialsStartInRecord); % 1-based index in MATLAB
    assert(p0 >= 1 && p0 + npar - 1 <= rowsize, ...
        "partialsStartInRecord makes partials block exceed record length.");

    % Most common layout (based on your comment): [extra][partials][igrps]
    % => partials indices in record: p0 : p0+npar-1
    partialIdxAll = p0 : (p0 + npar - 1);

    % Overwrite these specific positions
    overwriteIdx = partialIdxAll(parCols);
    ...
    while true
        ...
        % Ensure newPartials has enough rows
        assert(size(newPartials,1) >= nObs, ...
            "newPartials has only %d rows but file has at least %d observations.", ...
            size(newPartials,1), nObs);
        assert(size(newPartials,2) == numel(parCols), ...
            "newPartials must be Nobs x K where K=numel(parCols).");

        % Overwrite selected partial columns
        vals = newPartials(nObs, :).';
        rec(overwriteIdx+extra) = vals;

        fwrite(fout, rec, 'double');
    end

    % Final check: if user provided extra rows in newPartials, that's ok.
    info = struct();
    info.npar    = npar;
