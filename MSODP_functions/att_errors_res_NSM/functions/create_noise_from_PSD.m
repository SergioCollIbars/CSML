...
% %     win_len = min(floor(N/4), 10^5); % Use 1/8th of signal, capped for memory
    win_len = floor(N/4);
    nfft_val = 2^nextpow2(win_len * 2);
    [pxx, fwelch] = pwelch(x, hanning(win_len), floor(win_len/2), nfft_val, fs, 'onesided');
    ...
    if(plotCommand)
        figure;
        loglog(fwelch, sqrt(pxx), 'LineWidth', 1.5); hold on;
        loglog(fwelch, sqrt(S1_w), '--', 'LineWidth', 1.5);
        xlabel('Frequency [Hz]'); ylabel('PSD [units/Hz^{1/2}]');
