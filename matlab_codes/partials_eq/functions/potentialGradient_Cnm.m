function [H] = potentialGradient_Cnm(n_max, r, Re, GM, ...
    NB, normalized)
 %%                   COMPUTE ACCELERATION FUNCTION
    ...
    %
    %   Output: 
    %       H: visibility matrix. pratials respect to GM*Cnm
    %
    % --------------------------------------------------------------------%
    ...
    % Spherical partials matrix components
    ...
    for n = 0:n_max
        for m = 0:n
            ...
            % Spherical harmonics derivative components
            if(m == 0)
    ...
    H(:, 1) = [dxdx_dc(1, 1);dydx_dc(1, 1);dzdx_dc(1, 1);...
    ...
    % ensamble visibility matrix. S harmonics
    j = Nxc + 1;
    nl = 3;
    while j < Nxc + Nxs
        for i = 1:nl
