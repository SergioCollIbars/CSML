...
% gradiometer signal. Then, compute sensitivity and Fisher information
% matrix. Optimize based on orbit inclination and eccentricity.
%
% Date: 10/10/2025
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% plot results
figure()
plot(rad2deg(i), IF_inc, 'LineWidth', 2)
grid on;
...
function [Amplitude_nm,frequency_nm] = compute_Amplitude_Freq(n_max, ...
    ...
    rates = j2_secular_rates(a, e, i, GM, Ref, -Cmat(3,1));
        ...
        for m = 0:k
            ...
            for idx = 1:length(p_range)
                for q = -q_range/2:q_range/2
                    ...
                    c = r3_inv * (k+1)*(k+2)*(k+3)*GM * ((Ref/ a)^k) * (1 / a);
...
function [F, f_bins, A_sum_per_bin] = fisher_from_lines(f_mat, A_mat, T_orb, band, df)
...
%
% Returns:
%   F              : Fisher info for the single scalar parameter (here Δr)
...
%   A_sum_per_bin  : sum of amplitudes in each bin (Eotvos/m)
    ...
    Svals = 1E-24;
    F = (T_orb/2) * sum( (A_sum_per_bin.^2) ./ Svals );
end
