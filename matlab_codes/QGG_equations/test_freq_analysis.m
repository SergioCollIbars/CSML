...
for k = 1:N
    ...
    [Nnm] = nomalization_fnc(n, m);
    Cnm = Cmat(n+1, m+1); Snm = Smat(n+1, m+1);

    Trr(k) = Anm * Pnm * Nnm * (Cnm*cos(m*lambda) + Snm*sin(m*lambda));
