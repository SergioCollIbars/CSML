...
coefErr_1_R_batch_bias  = load("dataFiles/CoefErr_batch_1_R_bias.mat").CoefErr;
coefErr_1_R_batch_noise  = load("dataFiles/CoefErr_batch_1_R_noise_E12.mat").CoefErr;
...
% compute rotation matrices at nominal
...
% GM Error
deltaGM = [1E-7, 1E-6, 1E-4, 1E-3, 1E-2, 1E-1];
%deltaGM = 0;
N_errGM = length(deltaGM);
CoeffErr_f = zeros(N_errGM, n_max);
err_f = zeros(3*N_errGM, Nt_max);

for w = 1:N_errGM
    disp('GM error N: ' + string(w));
    % modify GM value
    GM = 5.2 + deltaGM(w);
    ...
    while(count <= maxCount)
        ...
        for k = 1:Nt_max
            ...
            % pre-weighting meas
            if(ODEST_convergence == 1)
                Y = [Y(5) - Y(9); Y(6); Y(4); Y(3)];
                Hp = [Hp(5, 2:end)-Hp(9, 2:end); Hp(6, 2:end);...
                    Hp(4, 2:end); Hp(3, 2:end)];
                R = eye(4) * sigma^2;
                
                % re-start counter when ODEST converge
                count = 1;
            else
                Y = [Y(5) - Y(9); Y(6)];
                Hp = [Hp(5, 2:end) - Hp(9, 2:end); Hp(6, 2:end)];
...
% %             rr_ACI, rr_ACAF, ACAF_RTN_mat, RTN_ACI_mat, Cnm_updt,...
% %             Snm_updt, Y_o, X, H_mat, Y_oc, XNOT);
% % 
...
% %         end
    ...
    CoeffErr_f(w, :) = CoefErr(count-1, :);
    % % err_f((w*3-2):w*3,:) = errj(down:up, :);
end
...
loglog(deltaGM, CoeffErr_f(:, 2), '--sq', 'lineWidth', 1.5)
hold all;
loglog(deltaGM, CoeffErr_f(:, 3), '--sq', 'lineWidth', 1.5)
loglog(deltaGM, CoeffErr_f(:, 4), '--sq', 'lineWidth', 1.5)
loglog(deltaGM, CoeffErr_f(:, 5), '--sq', 'lineWidth', 1.5)
loglog(deltaGM, CoeffErr_f(:, 6), '--sq', 'lineWidth', 1.5)
grid on;
legend('n = 2', 'n = 3', 'n = 4', 'n = 5', 'n = 6')
xlabel('\delta GM [m^3 / s^2]')
ylabel('Coefficient RMS error')
title('RMS coefficient error at different GM values')

figure();
err_f_m = abs(mean(err_f'));
for b = 1:3
    for c = 1:N_errGM
        subplot(3, 1, b);
        semilogx(deltaGM(c), err_f_m(c*3-(3-b)), 'sq');
        hold all;
    end
    grid on;
    xlabel('\delta GM [m^3 / s^2]')
...
sgtitle('Mean position error at different GM values');
...
for c = 1:count
    ...
    if(count ==  4) 
        hold on;
        %semilogy(nval, Coef_truth(1, 2:end), 'k -', LineWidth= lw);
        %semilogy(nval, coefErr_truth(c, 2:end), 'o --', LineWidth= lw);
        semilogy(nval, coefErr_truth_noise_E12(c, 2:end), 'o --', LineWidth= lw);
        %semilogy(nval, coefErr_1_R_batch_bias(c, 2:end), '* --', LineWidth= lw);
        %semilogy(nval, coefErr_1_R_batch_noise(c, 2:end), '* --', LineWidth= lw);
    end
...
if(count == 4)
    legend('ODEST', 'No deviation', 'Batch only');
    %legend('No deviation', 'ODEST');
    %legend('Truth values', 'ODEST', 'No deviation');
else
    legend('ODEST');
end
...
figure();
subplot(3, 1, 1);
plot(t, Aj(1, :), '--', t, ones(1, Nt_max) * delta_bias(1), LineWidth=1.5);
grid on;
xlabel('TIME [s]')
ylabel('\Delta_R');

subplot(3, 1, 2);
plot(t, Aj(2, :), '--', t, ones(1, Nt_max) * delta_bias(2), LineWidth=1.5);
grid on;
xlabel('TIME [s]')
ylabel('\Delta_T');

subplot(3, 1, 3);
plot(t, Aj(3, :), '--', t, ones(1, Nt_max) * delta_bias(3), LineWidth=1.5);
grid on;
xlabel('TIME [s]')
ylabel('\Delta_N');
sgtitle('ODEST error retrieval. Iteration = 1');

% absolute error deviation predicton
figure();
err = r_RTN - rr_RTN;
errR = (r_RTN - rr_RTN)./r_RTN;
subplot(3, 1, 1);
...
% create table
tt = [nval', CoefErr(count, 2:end)', coefErr_truth_noise_E12(end, 2:end)'];
T = array2table(tt,...
    'VariableNames',{'Degree','ODEST','No deviation orbit'});
disp(T);
