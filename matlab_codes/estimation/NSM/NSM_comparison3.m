...
attitude = zeros(3, Nt);                                       % nominal attitude [rad]
attitude_true = attitude + At;                                 % true attitude    [rad]
[angVel_true, angAcc_true, ~, ~] = compute_angularVals(attitude_true, dA_dt, ddA_ddt, t);
[angVel_nom, angAcc_nom, H_angVel, H_angAcc]   = ...
    compute_angularVals(attitude, zeros(3, Nt), zeros(3, Nt), t);

figure()
subplot(3, 2, 1)
plot(t./86400, attitude_true, 'LineWidth', 2)
legend('\Psi, yaw', '\theta, pitch', '\phi roll')
title('Actual attitude values')
subplot(3, 2, 3)
plot(t./86400, angVel_true, 'LineWidth', 2)
legend({'$\omega_3$', '$\omega_2$', '$\omega_1$'}, 'Interpreter', 'latex')
subplot(3, 2, 5)
plot(t./86400, angAcc_true, 'LineWidth', 2)
legend({'$\dot{\omega_3}$', '$\dot{\omega_2}$', '$\dot{\omega_1}$'}, 'Interpreter', 'latex')
subplot(3, 2, 2)
plot(t./86400, attitude, 'LineWidth', 2)
legend('\Psi, yaw', '\theta, pitch', '\phi roll')
title('Nominal attitude values')
subplot(3, 2, 4)
plot(t./86400, angVel_nom, 'LineWidth', 2)
legend({'$\omega_3$', '$\omega_2$', '$\omega_1$'}, 'Interpreter', 'latex')
subplot(3, 2, 6)
plot(t./86400, angAcc_nom, 'LineWidth', 2)
legend({'$\dot{\omega_3}$', '$\dot{\omega_2}$', '$\dot{\omega_1}$'}, 'Interpreter', 'latex')
...
while count < iterMax
    ...
    for j = 2:Nt
        ...
        [ax, nx] = nullSpace_method2(Y(:, j), Yc, attitude_true(:, j), attitude(:, j),...
            [Hc_BODY;zeros(3, Ncs)], R_N, [Hpos,Hrot], noise(:, j));
        Ax_N  = Ax_N + ax;
        Nx_N  = Nx_N + nx;

        % Null space method accounting for position only
    ...
    disp('Null space update, posittion + attitude = ' + string(vecnorm(XNOT_N)));
...
function [ax, nx] = nullSpace_method2(Y, Yc, att, attc, Hc, R, Hpr, noise)
    % add noise
    Y = Y + noise(1:9);

    % select measurements
    dY = [Y(1)-Yc(1);Y(2)-Yc(2);Y(3)-Yc(3);Y(5)-Yc(5);Y(6)-Yc(6);...
        att - attc + noise(10:12)];
...
function [angVel, angAcc, H_angVel, H_angAcc] = compute_angularVals(attitude, dA_dt, ddA_ddt, t)
    ...
    for j = 1:Nt
        theta = attitude(2, j); phi = attitude(3, j);
        thetaDot = dA_dt(2, j); phiDot = dA_dt(3, j);
        A = [-sin(theta), 0, 1;...
            sin(phi)*cos(theta), cos(phi), 0;...
            cos(phi)*cos(theta), -sin(phi), 0];
        ...
        finish = 3*j;
        init = finish - 2;
        H_angAcc(init:finish, :) = (1/dt).*A_dot + (1/dt^2).*A;
    end
end
