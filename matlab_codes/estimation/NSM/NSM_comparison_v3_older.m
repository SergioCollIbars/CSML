...
while count < iterMax
    ...
    for j = 1:Nt
        ...
        [Hrot] = compute_rotPartials(n_max, normalized, Cp_N, Sp_N, Re, GM, rn_ACI, ACAF_ACI, ACAF_ACI);
        ...
        [ax, nx] = nullSpace_method2(Y(:, j), Yc, [Hc_ACI;zeros(3, Ncs)], R_N, [Hpos,Hrot], eye(3,3), noise(:, j), At);
        Ax_N  = Ax_N + ax;
        Nx_N  = Nx_N + nx;

        % Null space method only accounting for position
        [Yc, Hc_ACI, ~] = gradiometer_meas(t(j) ,asterParams, poleParams, [rn(:, j)', vn(:, j)'], ...
                noise0, Cp_NP, Sp_NP, eye(3,3));
        [Hpos] = compute_posPartials(n_max, normalized, Cp_NP, Sp_NP, Re, GM, rn_ACI, ACAF_ACI, ACAF_ACI);
        [ax, nx] = nullSpace_method(Y(:, j), Yc, Hc_ACI, R_NP, Hpos, eye(3,3), noise(:, j), At);
        Ax_NP  = Ax_NP + ax;
        Nx_NP  = Nx_NP + nx;
    end
...
function [ax, nx] = nullSpace_method(Y, Yc, Hc, R, Hp, ACI_RTN, noise, At)
    ...
    [Rot] = rotationMatrix(At(1), At(2), At(3), [1, 2, 3]); % BN
...
function [ax, nx] = nullSpace_method2(Y, Yc, Hc, R, Hpr, ACI_RTN, noise, At)
    ...
    ddU = [Y(1),Y(2),Y(3);Y(4),Y(5),Y(6);Y(7),Y(8),Y(9)];
    ddUc = [Yc(1),Yc(2),Yc(3);Yc(4),Yc(5),Yc(6);Yc(7),Yc(8),Yc(9)];
    [Rot] = rotationMatrix(At(1), At(2), At(3), [1, 2, 3]);
