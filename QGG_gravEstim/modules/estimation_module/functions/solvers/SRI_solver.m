function [varObj, XNOT] = SRI_solver(varObj, count, GG_meas, dynamic_meas,...
    ...
    for k =1:varObj.Nt
        ...
        PHI_ji = inv(PHI_ij);
          ...
          % compute visibility matrix. Gradiometer
         ...
         [he_t, bias] = H_extraParam(varObj, k);
         ...
         % pre-weighting measurements
         dY = pW\dY;
         Hi_t = pW\Hi_t;

         % run filter
        [R, b] = SRIF(dY, Hi_t, R, b, PHI_ji);

        % trasform from information domain to covariance
        P = inv(R'*R);
