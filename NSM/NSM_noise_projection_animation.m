...
sigma_RR = sqrt(10); sigma_RT= sqrt(1); sigma_RN = sqrt(1); sigma_TT = sqrt(1); sigma_TN = sqrt(1); sigma_NN = sqrt(1);
R = diag([sigma_RR, sigma_RT, sigma_RN, sigma_TT, sigma_TN].^2);

means    = zeros(1, 5);
std_devs = [sigma_RR, sigma_RT, sigma_RN, sigma_TT, sigma_TN];
num_realizations = Nr; % Number of realizations

x_corr  = zeros(Nr, Nt); y_corr  = x_corr;
x_dcorr = zeros(Nr, Nt); y_dcorr = x_dcorr;

% projected std over time
STD_proj = ones(2, length(t));
for k = 1:length(t)
    ...
    % save noise projection
    STD_proj(1, k) = var(nd(1, :));
    STD_proj(2, k) = var(nd(2, :));
end

% plot projected STD over time
figure()
plot(t, STD_proj, 'LineWidth', 2);
legend('\lambda_1', '\lambda_2');
title('STD values of the projected noise');
xlabel('TIME [sec]');
ylabel('[1/s^2]');

% do-video
createVideo(x_corr, y_corr, x_dcorr, y_dcorr, t, vecnorm(rn), Nt, 30);
...
function [] = createVideo(x1, y1, x2, y2, time, pos, N,axlim)
    ...
    for t = 1:N
        ...
        title('De-correlated')
        xlabel('\lambda_1 - axis');
        ylabel('\lambda_2 - axis');
        grid on;
        axis equal;  % Equal scaling on both axes
        axis([-axlim axlim -axlim axlim]);
