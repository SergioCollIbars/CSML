...
for Mc = 1:mc_num
    ...
    delta0R = normrnd(0, sigmaP, [3, 1]);
    deltaX = normrnd(0, sigmaCoeff, [Ncs, 1]);

    X0 = [r0+delta0R;v0];
    deltaX(1) = 0;
    [Cp, Sp] = list2mat(n_max, Nc, Ns, X+deltaX);
    % % P0_LS = diag([1E3*deltaX(2:end); 1.*ones(6, 1)].^2);
    P0_LS = diag([deltaX(2:end); sigmaP*ones(6, 1)].^2);
    P0_NSM = diag(100*deltaX(2:end).^2);
    ...
    if(Solver == "LS")
        % solve LS (batch approach)
        N_batch = round(rev*T/ 60 / 3);    % solve every 30 min
        index   = floor(Nt/ N_batch);
        X0_LS   = X0; Cp_LS = Cp; Sp_LS = Sp;
        for k = 1:N_batch
            fprintf('Solve batch from index %.3f to %.3f \n', ...
                index*k-(index -1), index*k);
            t_batch = t(index*k-(index -1):index*k);
            if(k == N_batch)
                t_next = t_batch;
            else
                t_next  = t(index*k-(index -1):index*k+1);
            end
            [X_LS, P_LS, P_final, r_final, v_final] = solver_LS(t_batch, t_next, X0_LS(1:3),...
                X0_LS(4:6), Cp_LS, Sp_LS, asterParams, poleParams, P0_LS, ...
                Y_true(:,index*k-(index-1):index*k), R);
            
            % update states at time = (index*k + 1)
            P0_LS = P_final; 
            X0_LS = [r_final;v_final];
            [Cp_LS, Sp_LS] = list2mat(n_max, Nc, Ns, X_LS(1:Ncs));
        end
        P3  = P_LS;
        dX3(:, Mc) = X_LS;
    elseif(Solver == "NSM")
        deltaR = normrnd(0, sigmaP, [3, Nt]);
        rn = state_t(:, 1:3)' + deltaR;
        vn = state_t(:, 4:6)';
...
elseif(Solver == "LS")
    ...
    Perr  = abs(dX3(end-5:end, :) - state_t(index*k-(index-1), 1:6)');
    sigma3 = sqrt(diag(P3));
    ...
    legend('\sigma Co-estim', 'err Co-estim');
    grid on;
end

%% FUNCTIONS

function [X0_final, P0_final, P_final, r_final, v_final] = solver_LS(t, t_new, rn0, vn0, Cnm, Snm, asterParams, ...
    ...
    % integrate trajectory to final
    [~, state_n] = ode113(@(t, x) EoM(t, x, Cnm, Snm, n_max, GM, Re, normalized, ...
            W0, W, RA, DEC, 1), t_new, [rn0;vn0;STM0], options);
    r_final = state_n(end, 1:3)'; v_final = state_n(end, 4:6)';
    
    % STM from t_final to t0
    PHI = reshape(state_n(end, 7:end), [Ncs+5, Ncs+5]); 

    P_final = PHI * (Ax_L \ PHI');
end
