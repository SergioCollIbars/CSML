function [Hc_BODY] = rotate_coeffPartials(Hc_ACI,BN)
    ...
    % fill-out 00 term
    j = 1;
    TN = [h(1, j), h(2, j), h(3, j);h(4, j),h(5, j),h(6, j);...
        h(7, j),h(8, j), h(9, j)];
    TB = BN * TN * BN'; % rotate to body frame
    Hc_BODY(:, 1) = [TB(1,1);TB(1,2);TB(1,3);TB(2,1);TB(2,2);TB(2,3);...
        TB(3,1);TB(3,2);TB(3,3)];

    % fill-out rest of the terms
    for j = 2:length(Hc_ACI(1, :))
        TN = [h(1, j), h(2, j), h(3, j);h(4, j),h(5, j),h(6, j);...
                  h(7, j),h(8, j), h(9, j)];
        TB = BN * TN * BN'; % rotate to body frame
        Hc_BODY(:, j) = [TB(1,1);TB(1,2);TB(1,3);TB(2,1);TB(2,2);TB(2,3);...
        TB(3,1);TB(3,2);TB(3,3)];
    end
end
